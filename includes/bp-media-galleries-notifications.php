...
function bp_media_galleries_send_high_five_notification( $to_user_id, $from_user_id ) {
	global $bp;
	...
	if ( 'no' == get_usermeta( (int)$to_user_id, 'notification_media_galleries_new_high_five' ) )
		return false;
	...
	$sender_highfive_link = site_url( BP_MEMBERS_SLUG . '/' . $sender_ud->user_login . '/' . $bp->media_galleries->slug . '/view' );
	...
	$subject = '[' . get_blog_option( 1, 'blogname' ) . '] ' . sprintf( __( '%s high-fived you!', 'bp-media-galleries' ), stripslashes($sender_name) );
...
---------------------
', 'bp-media-galleries' ), $sender_name, $sender_name, $sender_profile_link, $sender_name, $sender_highfive_link );

	$message .= sprintf( __( 'To disable these notifications please log in and go to: %s', 'bp-media-galleries' ), $reciever_settings_link );

	// Send it!
	wp_mail( $to, $subject, $message );
}
add_action( 'bp_media_galleries_send_high_five', 'bp_media_galleries_send_high_five_notification', 10, 2 );
