...
class BP_Media_Galleries_Gallery_Template {
	var $current_item = -1;
	var $item_count;
	var $items;
	var $item;
	...
	function bp_media_galleries_template( $user_id, $type, $page, $per_page, $max ) {
		global $bp;
		...
		switch ( $type ) {
			case 'newest':
				$this->items = bp_media_galleries_get_newest( $user_id, $this->pag_num, $this->pag_page );
				break;
			case 'popular':
				$this->items = bp_media_galleries_get_popular( $user_id, $this->pag_num, $this->pag_page );
				break;
			case 'alphabetical':
				$this->items = bp_media_galleries_get_alphabetical( $user_id, $this->pag_num, $this->pag_page );
				break;
		}
		/**/
	...
	function user_items() {
		if ( $this->current_item + 1 < $this->item_count ) {
			...
			do_action('bp_media_galleries_loop_end');
	...
	function the_item() {
		...
		if ( 0 == $this->current_item ) // loop has just started
			do_action('bp_media_galleries_loop_start');
	}
}

function bp_media_galleries_has_items( $args = '' ) {
	 ...
	 *
	 * e.g. bp_get_media_galleries_has_items( 'per_page=10&max=50' );
	 */

	/***
	 * Set the defaults for the parameters you are accepting via the "bp_get_media_galleries_has_items()"
	 * function call
	 */
	$defaults = array(
		'user_id' => false,
	...
	$items_template = new BP_Media_Galleries_Gallery_Template( $user_id, $type, $page, $per_page, $max );

	return $items_template->has_items();
}

function bp_media_galleries_the_item() {
	global $items_template;
	return $items_template->the_item();
}

function bp_media_galleries_items() {
	global $items_template;
	return $items_template->user_items();
}

function bp_media_galleries_item_name() {
	echo bp_media_galleries_get_item_name();
}
	/* Always provide a "get" function for each template tag, that will return, not echo. */
	function bp_media_galleries_get_item_name() {
		global $items_template;
		echo apply_filters( 'bp_media_galleries_get_item_name', $items_template->item->name ); // Example: $items_template->item->name;
	}

function bp_media_galleries_item_pagination() {
	echo bp_media_galleries_get_item_pagination();
}
	function bp_media_galleries_get_item_pagination() {
		global $items_template;
		return apply_filters( 'bp_media_galleries_get_item_pagination', $items_template->pag_links );
	}
